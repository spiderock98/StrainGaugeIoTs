...
async def connect():
    ...
    #     await sio.emit('auth', (queryCamID, password, uniqueSensorID), namespace='/VideoStream')
    #     time.sleep(2)
    ...
    while(True):
        ...
        if flagStream:
            await sio.emit('stream', str(base64.b64encode(dataImg),
                                         'utf-8'), namespace='/VideoStream')
        time.sleep(0.1)
    vs.stop()
...
async def on_message(dictCross):
    global flagStream
    print(dictCross)
...
async def main():
    await sio.connect('http://localhost:8080/', namespaces=['/VideoStream'])
